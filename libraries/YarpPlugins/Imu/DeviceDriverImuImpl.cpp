...
bool IMUdevice::open(yarp::os::Searchable & config)
{
     ...
     period = config.check("period", yarp::os::Value(DEFAULT_PERIOD), "IMU Period").asFloat64();
     frequency = config.check("freq",yarp::os::Value(DEFAULT_FREQUENCY), "Frequency").asInt32();
     ...
     if(config.check("period")){
         ...
         frequency = config.find("freq").asInt32();
         period = 1 / frequency ;
         cmcPeriod = period;
         cout << "Using period = " << period << "s, and frequency = " << frequency << "Hz." << endl;
     }else if(config.check("cmcPeriod")){
         cmcPeriod = config.find("cmcPeriod").asFloat64();
         period = cmcPeriod;
         frequency = 1 / period ;
         cout << "Using period = " << period << "s, and frequency = " << frequency << "Hz." << endl;
     }else {
