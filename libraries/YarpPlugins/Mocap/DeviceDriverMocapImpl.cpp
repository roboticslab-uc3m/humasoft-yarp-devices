...
bool MOCAPdevice::open(yarp::os::Searchable & config)
{
    ...
    sensoredRigidBodyID = config.check("rigidBodyID",yarp::os::Value(DEFAULT_RIGIDBODYID), "ID of the desired sensored rigidBody").asInt32();
    calibrNumSamples = config.check("calibrate",yarp::os::Value(DEFAULT_CALIBRNUMSAMPLES), "Number of frames to consider to calculate the calibration").asInt32();
