...
public:
    ...
    bool threadInit() override
    {
        ...
        if (!ok)
        {
            yError() << "Problems acquiring soft-neck interface";
            return 1;
        }
...
int main(int argc, char *argv[])
{
    ...
    ThreadVelocityControl velocityControl(0.05);
    velocityControl.start();

    yarp::os::Time::delay(0.2);
    printf("Thread Period: %f\n", velocityControl.getEstimatedPeriod() );

    printf("Press a button to stop the motor\n");

    getchar();

    velocityControl.stop();
